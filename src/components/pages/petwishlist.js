...
import FavPetsCard from "../card/FavPetsCard";
...
export const Petwishlist = () => {
  ...
  const loadPets = () => {
    user &&
      user.token &&
      getLovedPets(user.token).then((res) => {
        setloading(false);
  ...
  const removePet = (token, petid, petname) => {
    removeLovedPet(token, petid).then((res) => {
  ...
  return (
    <div className="container">
      {pets && pets.length > 0 ? (
        ...
        <center>
          <h1>You Don 't Have Any Pet On Your WishList </h1> <br />
          <br />
          <br />
          <br />
          <br />
        ...
        <div className="row">
          {pets.map((p) => (
            <div key={pets._id} className=" col-md-3 m-5">
              ...
              <FavPetsCard p={p} removePet={removePet} token={token} />
            </div>
          ))}
        </div>
      )}
